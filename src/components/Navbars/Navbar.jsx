...
function Navbar() {
  ...
  return (
    <>
      <header className="w-full min-w-[240px] z-30">
        <div className="py-1 flex items-center gap-2 sm:gap-4 justify-between px-4 sm:px-7 z-20 lg:px-10 primary relative">
          <Link to="/" className="flex items-center gap-3 shrink-0 outline-none" onClick={() => setIsHome(true)}>
            <img src={logo} alt="logo" className="sm:h-12 h-8 w-auto aspect-square" />
          ...
          <div className="flex gap-5 items-center">
            ...
            <ul className="flex items-center gap-2 sm:gap-4">
              <li className="flex items-center">
                <Link to="/cart" className="relative w-11 h-11 sm:w-auto sm:h-auto flex outline-none items-center">
                  ...
                  <div className="absolute sm:-top-2 top-0 bg-[#e18544] rounded-full px-1 left-6 text-white">{cart === 0 ? '' : cart}</div>
                </Link>
              </li>
              <li className="hidden sm:flex items-center">
                <Link to="/admin" className="relative flex items-center outline-none" onClick={() => setIsHome(false)}>
                  ...
                  <div className="absolute sm:-top-2 -top-0 bg-[#e18544] rounded-full px-1 left-5 text-white">{cart === 0 ? '' : cart}</div>
                </Link>
              </li>
              <li className="flex items-center sm:hidden relative w-11 h-11 sm:w-auto sm:h-auto outline-none">
                <svg viewBox="0 -960 960 960" fill="none" className="fill-[#e18544] h-9 w-9"><path d="M120-240v-80h720v80H120Zm0-200v-80h720v80H120Zm0-200v-80h720v80H120Z" /></svg>
                <div className="absolute sm:-top-2 -top-0 bg-[#e18544] rounded-full px-1 left-7 text-white"></div>
              </li>
            </ul>
          </div>
        </div>
